  ...
  with tf.Graph().as_default():
    ...
    for cpu_index in range(0, cpu_num):
        ...
        logit = c3d_model.inference_c3d(
                        images_placeholder[cpu_index * FLAGS.batch_size:(cpu_index + 1) * FLAGS.batch_size,:,:,:,:],
                        0.5,
                        FLAGS.batch_size,
                        weights,
                        biases
                        )
        loss_name_scope = ('cpud_%d_loss' % cpu_index)
        loss = tower_loss(
                        loss_name_scope,
                        logit,
                        labels_placeholder[cpu_index * FLAGS.batch_size:(cpu_index + 1) * FLAGS.batch_size]
                        )
    ...
    for step in xrange(FLAGS.max_steps):
      ...
      train_images, train_labels, _, _, _ = input_data.read_clip_and_label(
                      ...
                      batch_size=FLAGS.batch_size * cpu_num,
