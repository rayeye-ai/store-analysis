...
def get_train_test_lists(version='01'):
    ...
    test_file = os.path.join('list', 'test' + version + '.txt')
    train_file = os.path.join('list', 'train' + version + '.txt')
...
def move_files(file_groups):
    ...
    for group, videos in file_groups.items():
        ...
        for video in videos:
            ...
            classname = parts[0]
            filename = "output/" + parts[0] + os.path.sep + parts[1]
            ...
            # Move it
            try:
                os.makedirs(os.path.join(group, "output", classname))
            except:
                pass
